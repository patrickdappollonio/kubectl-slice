...
func (s *Split) parseYAMLManifest(contents []byte) (*kubernetes.YAMLFile, error) {
	...
	if err := yaml.Unmarshal(contents, &manifest); err != nil {
		return nil, fmt.Errorf("unable to parse YAML file number %d: %w", s.fileCount, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("unable to render file name for YAML file number %d: %w", s.fileCount, err)
	}
	...
	if k8smeta.Kind == "" && !s.opts.AllowEmptyKinds && (hasIncluded || hasExcluded) {
		return nil, &errors.CantFindFieldErr{FieldName: "kind", FileCount: s.fileCount, Meta: k8smeta}
	}
	...
	if k8smeta.Name == "" && !s.opts.AllowEmptyNames && (hasIncluded || hasExcluded) {
		return nil, &errors.CantFindFieldErr{FieldName: "metadata.name", FileCount: s.fileCount, Meta: k8smeta}
	}
	...
	if hasExcluded || hasIncluded {
		...
		if hasIncluded && !inSliceIgnoreCaseGlob(s.opts.Included, fmt.Sprintf("%s/%s", k8smeta.Kind, k8smeta.Name)) {
			return nil, &errors.SkipErr{Kind: "kind/name", Name: fmt.Sprintf("%s/%s", k8smeta.Kind, k8smeta.Name)}
		}
	...
	if len(s.opts.IncludedGroups) > 0 || len(s.opts.ExcludedGroups) > 0 {
		if k8smeta.APIVersion == "" {
			return nil, &errors.CantFindFieldErr{FieldName: "apiVersion", FileCount: s.fileCount, Meta: k8smeta}
		}
	...
	if str := strings.TrimSuffix(name, filepath.Ext(name)); str == "" {
		return nil, fmt.Errorf("file name rendered will yield no file name for YAML file number %d (original name: %q, metadata: %v)", s.fileCount, name, k8smeta)
	}
